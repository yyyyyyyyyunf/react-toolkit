...
# 检测 packages/feature-detector 的变化（排除 package.json 版本变化）
FEATURE_DETECTOR_CHANGES=$(git diff --name-only $BASE_BRANCH...$CURRENT_BRANCH | grep -E "^packages/feature-detector/" | grep -v "package.json" | wc -l)
...
echo "  packages/feature-detector: $FEATURE_DETECTOR_CHANGES 个文件变化"
...
if [ $FEATURE_DETECTOR_CHANGES -gt 0 ]; then
    if [ -n "${GITHUB_OUTPUT:-}" ]; then
        echo "FEATURE_DETECTOR_CHANGED=true" >> "$GITHUB_OUTPUT"
    fi
    echo "✅ @fly4react/feature-detector 需要更新版本"
else
    if [ -n "${GITHUB_OUTPUT:-}" ]; then
        echo "FEATURE_DETECTOR_CHANGED=false" >> "$GITHUB_OUTPUT"
    fi
    echo "❌ @fly4react/feature-detector 无需更新版本"
fi

# 如果有任何变化，根目录也需要更新
if [ $OBSERVER_CHANGES -gt 0 ] || [ $MEMO_CHANGES -gt 0 ] || [ $IMAGE_CHANGES -gt 0 ] || [ $FEATURE_DETECTOR_CHANGES -gt 0 ] || [ $ROOT_CHANGES -gt 0 ]; then
    if [ -n "${GITHUB_OUTPUT:-}" ]; then
...
echo ""
echo "🎯 发布策略:"
if [ $OBSERVER_CHANGES -gt 0 ] && [ $MEMO_CHANGES -gt 0 ] && [ $IMAGE_CHANGES -gt 0 ] && [ $FEATURE_DETECTOR_CHANGES -gt 0 ]; then
    echo "  📦 发布四个包的新版本"
elif [ $OBSERVER_CHANGES -gt 0 ] && [ $MEMO_CHANGES -gt 0 ] && [ $IMAGE_CHANGES -gt 0 ]; then
    echo "  📦 发布三个包的新版本"
...
elif [ $OBSERVER_CHANGES -gt 0 ] && [ $FEATURE_DETECTOR_CHANGES -gt 0 ]; then
    echo "  📦 发布 @fly4react/observer 和 @fly4react/feature-detector"
elif [ $MEMO_CHANGES -gt 0 ] && [ $IMAGE_CHANGES -gt 0 ]; then
    echo "  📦 发布 @fly4react/memo 和 @fly4react/image"
elif [ $MEMO_CHANGES -gt 0 ] && [ $FEATURE_DETECTOR_CHANGES -gt 0 ]; then
    echo "  📦 发布 @fly4react/memo 和 @fly4react/feature-detector"
elif [ $IMAGE_CHANGES -gt 0 ] && [ $FEATURE_DETECTOR_CHANGES -gt 0 ]; then
    echo "  📦 发布 @fly4react/image 和 @fly4react/feature-detector"
...
elif [ $FEATURE_DETECTOR_CHANGES -gt 0 ]; then
    echo "  📦 只发布 @fly4react/feature-detector"
