...
import { calculateScrollDirection, createIntersectionObserver, uniqueId } from '../utils';
...
class IntersectionObserverManager {
   ...
   */
  private getObserver(options: ObserverOptions): IntersectionObserver | undefined {
      ...
      const observer = createIntersectionObserver(entries => {
        for (const entry of entries) {
          const elementUniqueId =
            entry.target.getAttribute('data-intersection-unique-id') || 'intersection_element_void';
  ...
  observe(
    ...
    const elementUniqueId =
      target.getAttribute('data-intersection-unique-id') || uniqueId('intersection_element');
   ...
   * @param options 配置选项
   */
  unobserve(target: Element, options: ObserverOptions = {}, callback?: ObserverCallbackType): void {
    if (!target) return;

    const elementUniqueId =
      target.getAttribute('data-intersection-unique-id') || 'intersection_element_void';
    ...
    for (const [_, callbacks] of this.callbacks) {
      if (callbacks.has(elementUniqueId) && callbacks.get(elementUniqueId)?.size) {
