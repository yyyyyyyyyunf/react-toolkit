...
export class FeatureDetector<T extends FeatureConfigMap = FeatureConfigMap> {
  ...
  constructor(initialConfig: T, detectorConfig: FeatureDetectorOptions = {}) {
    ...
    // 合并配置，使用默认值作为后备
    this.browserPatterns = detectorConfig.browserPatterns || defaultDetectorConfig.browserPatterns;
    this.versionExtraction = {
      ...defaultDetectorConfig.versionExtraction,
      ...detectorConfig.versionExtraction,
    };
    this.webViewBrowsers = detectorConfig.webViewBrowsers || defaultDetectorConfig.webViewBrowsers;
  }
  ...
  detect<K extends keyof T | string>(features: K[], ua: string): DetectionResult[];
  ...
  check<K extends keyof T | string>(featureOrFeatures: K | K[], ua: string): boolean | boolean[] {
    if (Array.isArray(featureOrFeatures)) {
      return featureOrFeatures.map(feature => this.detectSingle(feature, ua).supported);
    }
   ...
   * ```
   */
  checkAsRecord<K extends keyof T | string>(features: K[], ua: string): Record<K, boolean> {
  ...
  registerFeature(name: string, config: FeatureConfig): void {
    const isUpdate = this.dynamicFeatures.has(name) || name in this.featureConfig;
  ...
  // 内部实现方法
  private detectSingle<K extends keyof T | string>(feature: K, ua: string): DetectionResult {
  ...
  private getFeatureConfig(feature: string | keyof T): FeatureConfig | null {
    return (
      this.dynamicFeatures.get(String(feature)) || this.featureConfig[feature as keyof T] || null
    );
  }

  private checkByUA(_feature: string, ua: string, config: FeatureConfig): boolean {
  ...
  private doubleCheck(feature: string, ua: string, config: FeatureConfig): DetectionResult {
    ...
    for (const pattern of this.browserPatterns) {
      ...
      for (const regex of pattern.patterns) {
        ...
        if (match) {
          ...
          // 判断是否为 WebView
          const isWebView = this.webViewBrowsers.some(name => name === pattern.name);
  ...
  private checkBrowserSupport(browserInfo: BrowserInfo, browsers: BrowserSupport): boolean {
 ...
 * ```
 */
export function createSSRDetector(config?: Partial<FeatureConfigMap>): FeatureDetector {
  const featureConfig: FeatureConfigMap = { ...defaultFeatureConfigs };
  if (config) {
    for (const key in config) {
      const value = config[key];
