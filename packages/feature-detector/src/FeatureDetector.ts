...
import {
  ...
  export class FeatureDetector<T extends FeatureConfigMap = FeatureConfigMap> {
    ...
    constructor(
      ...
      this.versionExtraction = { ...defaultDetectorConfig.versionExtraction, ...detectorConfig.versionExtraction };
    }
    ...
    private getFeatureConfig(feature: string | keyof T): FeatureConfig | null {
      return this.dynamicFeatures.get(String(feature)) || 
    ...
    private doubleCheck(feature: string, ua: string, config: FeatureConfig): DetectionResult {
      ...
      try {
        ...
        } else if (!uaSupport && !runtimeSupport) {
          return {
            supported: false,
            method: 'ua-runtime',
            uaSupport: false,
    ...
    private detectBrowser(ua: string): BrowserInfo | null {
      // 检测浏览器
      for (const pattern of this.browserPatterns) {
        ...
        if (match) {
          ...
          return {
            ...
            isWebView: pattern.name === 'safariWebview' || pattern.name === 'chromeWebview',
            webkitVersion,
            userAgent: ua
          };
        }
    ...
    private normalizeVersion(version: string): string {
      const match = this.versionExtraction.versionPattern ? version.match(this.versionExtraction.versionPattern) : null;
