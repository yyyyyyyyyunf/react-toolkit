import { FeatureConfigMap, BrowserPattern, DetectorConfig } from './types';
...
export const defaultBrowserPatterns: BrowserPattern[] = [
  ...
  {
    name: 'chromeWebview',
    pattern: /chrome\/([\d.]+).*(?:wv|version\/[\d.]+)/i, // 包含 wv 或 Version/ 字段
    versionIndex: 1
  },
  {
    name: 'safariWebview',
  ...
  {
    name: 'edge',
    pattern: /edg(?:e|ios|a)?\/([\d.]+)/i,
    versionIndex: 1
  },
  {
    name: 'chrome',
    pattern: /chrome\/([\d.]+)(?!.*(?:wv|version\/))/i, // 确保不是 WebView（不包含 wv 或 Version/）
    versionIndex: 1
  },
  {
    name: 'firefox',
    pattern: /(?:firefox|fxios)\/([\d.]+)/i,
    versionIndex: 1
  },
  {
    name: 'opera',
    pattern: /(?:opera|opr)[\s/]([\d.]+)/i,
    versionIndex: 1
  },
  {
    name: 'samsung',
  ...
  {
    name: 'safari',
    pattern: /version\/([\d.]+).*safari(?!.*chrome)/i, // 确保不是 Chrome
    versionIndex: 1
  }
];
