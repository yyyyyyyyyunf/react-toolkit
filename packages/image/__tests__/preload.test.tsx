...
import { isImageInPreloadQueue, addToPreloadQueue, clearPreloadQueue, getPreloadQueue, PreloadOptions } from '../src/utils/preload';
...
describe('Image Preload', () => {
	...
	describe('Preload Queue Utilities', () => {
		it('应该检查图片是否在预加载队列中', () => {
			...
			const isInQueue = testQueue.some((item) => item.src === 'https://example.com/image.jpg');
			expect(isInQueue).toBe(true);
			
			const isNotInQueue = testQueue.some((item) => item.src === 'https://example.com/other-image.jpg');
		...
		it('应该避免重复添加相同的图片', () => {
			...
			// 测试去重逻辑
			const uniqueQueue = testQueue.filter((item, index, self) => 
	...
	describe('SSR Utilities', () => {
		it('应该生成预加载 HTML', () => {
			...
			expect(html).toContain('<link rel="preload" as="image/jpeg" href="https://example.com/image1.jpg" importance="high">');
			expect(html).toContain('<link rel="preload" as="image/png" href="https://example.com/image2.jpg" importance="low">');
		});
