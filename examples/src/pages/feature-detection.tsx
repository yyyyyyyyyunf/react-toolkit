...
import { createFeatureDetector, FeatureConfigMap } from '@fly4react/feature-detector';
...
export default function FeatureDetectionExample() {
  ...
  const predefinedFeatures = [
    ...
    { name: 'intersection-observer', description: 'Intersection Observer API 支持' },
  ...
  return (
    <div className="example-container">
      <h2>特性检测示例</h2>
      <p>使用 <code>@fly4react/feature-detector</code> 检测浏览器对特定特性的支持情况。</p>
          ...
          {browserInfo.browserInfo && (
            <>
              <div className="info-item">
                ...
                <code>{browserInfo.browserInfo.name} {browserInfo.browserInfo.version}</code>
              </div>
              <div className="info-item">
                <strong>是否 WebView：</strong>
                <span className={browserInfo.browserInfo.isWebView ? 'status-enabled' : 'status-disabled'}>
          ...
          {predefinedFeatures.map(feature => (
            <div key={feature.name} className="feature-card">
              <div className="feature-header">
                <h4>{feature.name}</h4>
                <span className={`status-badge ${features[feature.name] ? 'supported' : 'not-supported'}`}>
              ...
              {detailedResults[feature.name] && (
                <div className="feature-details">
                  ...
                  {detailedResults[feature.name].browserInfo && (
                    <div className="detail-item">
                      <strong>浏览器：</strong>
                      <code>{detailedResults[feature.name].browserInfo.name} {detailedResults[feature.name].browserInfo.version}</code>
                    </div>
                  )}
                </div>
              )}
              ...
              {customResult.error ? (
                ...
                <div className="result-details">
                  <div className="result-item">
                    <strong>支持状态：</strong>
                    <span className={`status-badge ${customResult.supported ? 'supported' : 'not-supported'}`}>
                  ...
                  {customResult.browserInfo && (
                    <div className="result-item">
                      <strong>浏览器信息：</strong>
                      <code>{customResult.browserInfo.name} {customResult.browserInfo.version}</code>
                    </div>
                  )}
                </div>
              )}
      ...
      <div className="demo-section">
        ...
        <div className="code-example">
          <pre><code>{`import { createFeatureDetector } from '@fly4react/feature-detector';
...
console.log('批量结果:', results);`}</code></pre>
        </div>
      </div>
      ...
      <div className="demo-section">
        ...
        <ul className="tips-list">
          <li>使用 <code>check()</code> 方法进行快速检测，只返回布尔值</li>
          <li>使用 <code>detect()</code> 方法获取详细检测结果</li>
