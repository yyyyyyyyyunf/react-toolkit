import { useElementPosition, useInViewport, useOneOffVisibility } from '@fly4react/observer';
...
const InitialViewportTest = () => {
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <h1 style={{ color: '#333', marginBottom: '20px' }}>初始 Viewport 行为测试</h1>
      ...
      >
        ...
        >
          <h4 style={{ color: '#007acc', marginBottom: '10px' }}>useElementPosition 测试</h4>
          <div
          ...
          >
            {position ? (
              <>
                <div style={{ fontWeight: 'bold', color: '#4caf50' }}>✅ 已获得位置信息</div>
        ...
        >
          <h4 style={{ color: '#ff9800', marginBottom: '10px' }}>useInViewport 测试</h4>
          <div
            ref={inViewportRef}
            style={{
              width: '100%',
          ...
          >
            {isInViewport ? (
              <>
                <div style={{ fontWeight: 'bold', color: '#ff9800' }}>✅ 元素在视口中</div>
                <div>状态: 可见</div>
              </>
            ) : (
      ...
      >
        <h4 style={{ color: '#9c27b0', marginBottom: '10px' }}>useOneOffVisibility 测试</h4>
        <div
        ...
        >
          {hasBeenVisible ? (
            <>
              <div style={{ fontWeight: 'bold', color: '#9c27b0' }}>🎯 元素已可见过</div>
              <div>一次性触发完成</div>
            </>
          ) : (
      ...
      >
        ...
        >
          {logs.map((log, index) => (
            <div key={`log-${index}-${log.slice(0, 20)}`} style={{ marginBottom: '2px' }}>
              {log}
            </div>
          ))}
        </div>
      ...
      >
        ...
        <p>
          <strong>
            这证明了 Intersection Observer 在元素一开始就在 viewport 中时会立即触发回调！
          </strong>
        </p>
      </div>
    </div>
