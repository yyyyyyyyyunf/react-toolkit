		...
		<div className="example-container">
			...
			<div className="status-panel">
				<div className="status-item">
					...
					<span className={`status-value ${isCeiling1 ? "active" : "inactive"}`}>
						{isCeiling1 ? "已贴顶" : "未贴顶"}
					</span>
				</div>
				<div className="status-item">
					<span className="status-label">位置检测 (position = 200px):</span>
					<span className={`status-value ${isCeiling2 ? "active" : "inactive"}`}>
						{isCeiling2 ? "已达到" : "未达到"}
					</span>
				</div>
				<div className="status-item">
					<span className="status-label">超出检测 (position = -50px):</span>
					<span className={`status-value ${isCeiling3 ? "active" : "inactive"}`}>
						{isCeiling3 ? "已超出" : "未超出"}
					</span>
				</div>
			</div>
			...
			<div style={{ 
				...
				<div style={{ marginTop: "10px", fontSize: "10px", color: "#666" }}>
					<strong>说明：</strong>在移动设备上，Chrome 的地址栏和状态栏会影响 viewport 计算。
					元素变绿时，其顶部可能已经超出视口顶部，这是正常现象。
				</div>
			</div>
			...
			<div className="demo-section">
				...
				{/* 额外的内容区域，用于滚动 */}
				<div style={{ height: "800px", backgroundColor: "#f5f5f5", padding: "20px" }}>
					<h3>滚动区域</h3>
					<p>向下滚动查看元素状态变化</p>
					<div style={{ height: "600px", backgroundColor: "#e0e0e0", marginTop: "20px" }}>
						...
						<div style={{ padding: "20px" }}>
							<h4>使用说明：</h4>
							<ul>
								<li><strong>贴顶检测</strong>：当元素顶部到达视口顶部时，元素变绿</li>
								<li><strong>位置检测</strong>：当元素顶部到达距离顶部 200px 时，元素变橙</li>
								<li><strong>超出检测</strong>：当元素顶部超出视口顶部 50px 时，元素变红</li>
							</ul>
						</div>
					</div>
				</div>
      ...
      <div ref={ref2} className={isCeiling2 ? "at-position" : ""}>
        位置检测元素
			...
			<div className="features-section">
				<h3>特性</h3>
				<ul>
					<li><strong>精确检测</strong>：使用连续的 threshold 来精确检测元素位置</li>
					<li><strong>状态优化</strong>：只在状态改变时更新，避免频繁触发</li>
					<li><strong>灵活配置</strong>：可自定义比较位置，支持正负值</li>
					<li><strong>性能优化</strong>：直接使用 IntersectionObserver，无额外开销</li>
