    ...
    <div style={{ padding: '20px' }}>
      ...
      >
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: '12px', fontFamily: 'monospace' }}>
              ...
              <div>
                位置: ({formatPosition(position1).left}, {formatPosition(position1).top})
              </div>
              <div>
                滚动: ({formatPosition(position1).scrollX}, {formatPosition(position1).scrollY})
              </div>
          ...
          >
            ...
            <div style={{ fontSize: '12px', fontFamily: 'monospace' }}>
              ...
              <div>
                位置: ({formatPosition(position2).left}, {formatPosition(position2).top})
              </div>
              <div>
                滚动: ({formatPosition(position2).scrollX}, {formatPosition(position2).scrollY})
              </div>
          ...
          >
            ...
            <div style={{ fontSize: '12px', fontFamily: 'monospace' }}>
              ...
              <div>
                位置: ({formatPosition(position3).left}, {formatPosition(position3).top})
              </div>
              <div>
                滚动: ({formatPosition(position3).scrollX}, {formatPosition(position3).scrollY})
              </div>
        ...
        {showDetails && (
          <details open style={{ marginTop: '15px' }}>
            <summary style={{ cursor: 'pointer', fontWeight: 'bold' }}>📋 详细位置数据</summary>
            <div
              style={{
                background: '#2c3e50',
                color: '#ecf0f1',
            ...
            >
              {[
                ...
                <div key={name} style={{ marginBottom: '10px' }}>
                  <strong>{name}:</strong>{' '}
                  {pos ? JSON.stringify(formatPosition(pos), null, 2) : '未检测到'}
                </div>
              ))}
            </div>
          </details>
      ...
      <section style={{ marginBottom: '80px' }}>
        ...
        <p style={{ color: '#666', fontSize: '14px' }}>
          配置: step=0.1, throttle=16ms, forceCalibrate=true, calibrateInterval=2500ms
        </p>
        <div
          ref={element1Ref}
          style={{
      ...
      <section style={{ marginBottom: '80px' }}>
        <h3>⚡ 元素 2: 纯滚动计算模式</h3>
        <p style={{ color: '#666', fontSize: '14px' }}>
          配置: step=0.05, throttle=8ms, forceCalibrate=false, calibrateInterval=0ms
        </p>
        <div
          ref={element2Ref}
          style={{
            ...
            fontWeight: 'bold',
            transform: position2
              ? `scale(${0.95 + position2.intersectionRatio * 0.1})`
              : 'scale(0.95)',
          }}
        >
          <div style={{ textAlign: 'center' }}>
      ...
      <section style={{ marginBottom: '80px' }}>
        <h3>📦 基于自定义容器的位置跟踪</h3>
        <p style={{ color: '#666', fontSize: '14px' }}>
          配置: 自定义容器, step=0.25, throttle=50ms, forceCalibrate=true, calibrateInterval=5000ms
        </p>
