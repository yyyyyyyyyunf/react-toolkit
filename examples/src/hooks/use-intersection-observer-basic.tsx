...
export function UseIntersectionObserverExample() {
	const [status, setStatus] = useState<"waiting" | "visible" | "hidden">(
		"waiting",
	);
	const [details, setDetails] = useState<string>("");
	const [intersectionData, setIntersectionData] = useState<
		Partial<ObserverCallbackParamType>
	>({});
				...
				{intersectionData.boundingClientRect && (
					<details style={{ marginTop: "15px" }}>
						...
						>
							<div>
								<strong>是否相交:</strong>{" "}
								{intersectionData.isIntersecting ? "是" : "否"}
							</div>
							<div>
								<strong>相交比例:</strong>{" "}
								{intersectionData.intersectionRatio?.toFixed(3)}
							</div>
							<div>
