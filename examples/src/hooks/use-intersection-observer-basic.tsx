import { type ObserverCallbackParamType, useIntersectionObserver } from '@fly4react/observer';
...
export function UseIntersectionObserverExample() {
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      >
        ...
        {intersectionData.boundingClientRect && (
          <details style={{ marginTop: '15px' }}>
            <summary style={{ cursor: 'pointer', fontWeight: 'bold' }}>📊 详细数据</summary>
            <div
              style={{
                background: '#ffffff',
                padding: '10px',
            ...
            >
              <div>
                <strong>是否相交:</strong> {intersectionData.isIntersecting ? '是' : '否'}
              </div>
              <div>
                <strong>相交比例:</strong> {intersectionData.intersectionRatio?.toFixed(3)}
              </div>
              <div>
                <strong>滚动方向:</strong> {intersectionData.scrollDirection}
              </div>
              <div>
                <strong>时间戳:</strong> {intersectionData.time?.toFixed(2)}ms
              </div>
              <div>
                <strong>元素位置:</strong>
                top: {intersectionData.boundingClientRect?.top.toFixed(1)}, left:{' '}
                {intersectionData.boundingClientRect?.left.toFixed(1)}
              </div>
              <div>
                <strong>元素尺寸:</strong>
      ...
      >
        <div style={{ fontSize: '48px', marginBottom: '10px' }}>{getStatusIcon()}</div>
        <div>被观察的元素</div>
        <div style={{ fontSize: '14px', marginTop: '5px', opacity: 0.9 }}>{details}</div>
      </div>
