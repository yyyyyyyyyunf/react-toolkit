...
import { useOneOffVisibility, useIntersectionRatio } from 'react-intersection-tool';
...
export function AnimationTriggersExample() {
  ...
  // 计数动画 Hook
  const useCountUp = (isVisible: boolean, target: number, duration: number = 2000) => {
  ...
  }, [fadeInVisible, slideInVisible, scaleInVisible, rotateInVisible, bounceInVisible, countUpVisible]);
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {['fadeIn', 'slideIn', 'scaleIn', 'rotateIn', 'bounceIn', 'countUp'].map(name => (
                <span
                  key={name}
                  style={{
                    padding: '4px 8px',
                    borderRadius: '12px',
                    fontSize: '12px',
                    background: playedAnimations.has(name) ? '#d4edda' : '#f8d7da',
      ...
      <section style={{ marginBottom: '100px' }}>
        ...
        <div
          ...
          style={{
            ...
            fontSize: '20px',
            fontWeight: 'bold',
            transform: rotateInVisible ? 'rotate(0deg) scale(1)' : 'rotate(-180deg) scale(0.5)',
      ...
      <section style={{ marginBottom: '100px' }}>
        ...
        <div
          ...
          style={{
            ...
            animation: bounceInVisible ? 'bounceIn 1s cubic-bezier(0.25, 0.46, 0.45, 0.94)' : 'none',
          }}
        >
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '48px', marginBottom: '10px' }}>🏀</div>
        ...
        >
          ...
          <div style={{ marginBottom: '15px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
              <span>进度 A</span>
              <span>{progressRatio !== undefined ? Math.round(progressRatio * 100) : 0}%</span>
            </div>
            <div style={{
              width: '100%',
              height: '10px',
          ...
          <div style={{ marginBottom: '15px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
              <span>进度 B</span>
              <span>{progressRatio !== undefined ? Math.round(progressRatio * 80) : 0}%</span>
            </div>
            <div style={{
              width: '100%',
              height: '10px',
        ...
        >
          <div style={{ textAlign: 'center', width: '100%' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-around' }}>
              <div>
                <div style={{ fontSize: '36px', fontWeight: 'bold' }}>{count1.toLocaleString()}</div>
                <div style={{ fontSize: '14px', opacity: 0.8 }}>用户数</div>
              </div>
              <div>
                <div style={{ fontSize: '36px', fontWeight: 'bold' }}>{count2.toLocaleString()}</div>
                <div style={{ fontSize: '14px', opacity: 0.8 }}>项目数</div>
              </div>
              <div>
                <div style={{ fontSize: '36px', fontWeight: 'bold' }}>{count3.toLocaleString()}</div>
